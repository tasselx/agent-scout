...
`agent-scout` 是一个开源的通用 **web 搜索**工具，提供三种使用形态：
**CLI** 命令行、**MCP server**（接入任意 MCP 客户端）、**Agent Skill**（供 AI agent 调用）。
基于 Windsurf/Devin 服务端搜索，**纯 Rust** 编写，**macOS / Linux / Windows 三平台支持**，
**零配置即用**——只要本机用 Devin 或 Windsurf 登录过，就会自动识别登录凭证。
...
只要本机用过 Devin / Windsurf 登录，程序会自动识别登录凭证完成搜索，无需手动设置 API key。
...
暴露 `web_search` 工具（`query` / `limit` / `domain` / `mode`），
...
- 每次运行读取 key 失败、搜索失败、config 出错时写入 `[ERROR]`
- 搜索成功时写入 `[INFO]`
...
`0`=成功，`1`=错误，`2`=用法错误。诊断信息走 stderr，stdout 为纯 JSON。
